...
export default function SurveysPage() {
  ...
  async function add() {
    ...
    setTitle(""); setUrl(""); setClassId(""); setDueDate(""); setShowAdd(false)
    await load()
    setSaving(false)
  }
    ...
    <div className="min-h-screen bg-black/50 backdrop-blur-sm" dir="rtl">
      ...
      <div className="max-w-lg mx-auto px-4 py-5 space-y-4">
                        ...
                        <div className="space-y-3">
                          ...
                          {st.completed.length > 0 && (
                            <div>
                              ...
                              <div className="flex flex-wrap gap-1.5">
                                {st.completed.map(c => (
                                  <span key={c.id} className="bg-green-500/10 text-green-300/70 text-[11px] px-2 py-1 rounded-lg">{c.name}</span>
                                ))}
                              </div>
                            </div>
                          )}
        ...
        <p className="text-white/15 text-[10px] text-center pt-2">
          תשובות השאלון עצמן נשארות בגוגל פורמס — כאן רואים רק מי ענה ומי לא.
        </p>
      </div>
    </div>
  )
